...
describe('load', () => {
	...
	it('seeds the current name into the form', async () => {
		...
		expect(data.accountForm.data).toMatchObject({ name: ada.name });
	});
});

describe('update', () => {
	it('updates the signed-in user', async () => {
		const updateUser = async ({ body }: { body: { name?: string } }) => {
			await harness.db
				.update(user)
				.set({ name: body.name ?? ada.name, updatedAt: new Date() })
	...
	it('returns a validation failure without writing', async () => {
		...
		const rows = await harness.db
			...
			.where(eq(user.id, ada.id))
			.limit(1);
		expect(rows[0]).toMatchObject({ name: ada.name, email: ada.email });
	});
