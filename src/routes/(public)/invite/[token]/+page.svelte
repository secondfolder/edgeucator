...
<section>
	...
	{:else if data.state === 'sign-in-required'}
		<h1>{data.inviterName} wants to link with you</h1>
		<p>Sign in or create an account to see what they're proposing. You can still say no.</p>
		<!-- Not plain "Log in" / "Sign up": the (public) header already offers
		     links by those names, and two links with the same accessible name
		     going to different places is a real problem for anyone navigating by
		     link list, not just for a test's locator. -->
		<div class="actions">
			<wa-button href={`${resolve('/login')}?redirectTo=${encodeURIComponent(data.redirectTo)}`}>
				Log in to accept
			</wa-button>
			<wa-button
				appearance="outlined"
				href={`${resolve('/signup')}?redirectTo=${encodeURIComponent(data.redirectTo)}`}
			>
				Create an account
			</wa-button>
		</div>
	...
	{:else if data.state === 'confirm'}
		...
		<p>
			...
			{:else}
				They've set these, and they're in control of this link, so you can't change them here.
			{/if}
		</p>
