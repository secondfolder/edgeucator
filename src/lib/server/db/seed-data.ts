 ...
 * `git show 8945006^:src/lib/server/db/seed.ts`.
 *
 * Ids are hardcoded rather than generated so that re-running the seed is
 * idempotent and dev URLs stay stable across `npm run db:reset`.
 *
 * This file is listed in .prettierignore so each displayText entry stays on one
 * line instead of being reflowed into an unreviewable diff.
 */
